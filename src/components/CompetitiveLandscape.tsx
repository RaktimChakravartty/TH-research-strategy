      ...
      <div className="space-y-2 mb-5 flex-1">
        {brand.metrics.map(([k, v], i) => (
          <div key={i} className="flex justify-between items-baseline gap-3">
            ...
            <span className="typ-caption font-medium" style={{ color: 'var(--text-on-dark)' }}>{v}</span>
          </div>
        ))}
      </div>
      <div style={{ height: 1, background: 'var(--border-dark)', margin: '0 0 16px' }} />
      <div className="mb-3">
        <div className="flex items-center gap-1.5 mb-2">
          <Icon name="check-circle" size={14} style={{ color: 'var(--green)' }} />
          <span className="typ-eyebrow" style={{ color: 'var(--green)', fontSize: 11 }}>Strengths</span>
        </div>
        <div className="flex flex-wrap gap-1.5">{brand.strengths.map((s, i) => (
          <span key={i} className="typ-caption px-2.5 py-1" style={{ background: 'var(--green-bg)', borderRadius: 8, color: 'var(--green)' }}>{s}</span>
        ))}</div>
      </div>
      <div>
        <div className="flex items-center gap-1.5 mb-2">
          <Icon name="target" size={14} style={{ color: 'var(--gold)' }} />
          <span className="typ-eyebrow" style={{ color: 'var(--gold)', fontSize: 11 }}>Gaps</span>
        </div>
        <div className="flex flex-wrap gap-1.5">{brand.gaps.map((g, i) => (
          <span key={i} className="typ-caption px-2.5 py-1" style={{ background: 'var(--gold-bg)', borderRadius: 8, color: 'var(--gold)' }}>{g}</span>
        ))}</div>
      </div>
    </div>
  );
  ...
  return (
    <div ref={ref}>
      <div className="flex items-center gap-2 mb-3">
        <Icon name={icon} size={16} style={{ color: 'var(--text-on-dark-tertiary)' }} />
        <span className="typ-caption font-medium" style={{ color: 'var(--text-on-dark-secondary)' }}>{label}</span>
      </div>
      <div className="space-y-3">
        {items.map((it, idx) => (
          <div key={it.name} className="flex items-center gap-3">
            <span className="typ-caption w-28 shrink-0 text-right" style={{ color: 'var(--text-on-dark-secondary)' }}>{it.name}</span>
            {it.val === 0 ? (
              <span className="typ-caption italic" style={{ color: 'var(--text-on-dark-tertiary)' }}>Not reported</span>
            ) : (
              <div className="flex-1 h-7 overflow-hidden" style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 8 }}>
                <div className="h-full flex items-center justify-end pr-3 bar-grow" style={{ width: vis ? `${(it.val / max) * 100}%` : '0%', backgroundColor: it.color, borderRadius: 8, transitionDelay: `${idx * 120}ms` }}>
                  <span className="typ-caption font-medium text-white">{it.val}</span>
                </div>
              </div>
            )}
          </div>
    ...
    <section className="section-dark">
      <div className="section-pad text-center">
        <div ref={r1.ref} className={r1.cls}>
          <p className="typ-eyebrow" style={{ color: 'var(--accent)' }}>Competitive Landscape</p>
          ...
          <p className="typ-body-large mt-4" style={{ color: 'var(--text-on-dark-secondary)', maxWidth: '600px', margin: '16px auto 0' }}>
            No competitor currently holds both operational foundation and brand system. That's the gap.
          </p>
        </div>
      </div>
      <div className="section-pad-wide" style={{ paddingTop: 0 }}>
        ...
        <div ref={r3.ref} className={`mt-12 card-dark ${r3.cls}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            {COMP_BARS.map((b, i) => (
              <HBar key={b.label} label={b.label} icon={['hotel', 'instagram', 'rupee'][i]} items={[
                ...
                { name: 'Zostel', val: b.z, color: '#6E6E73' },
                { name: 'goSTOPS', val: b.g, color: '#86868B' },
              ]} />
            ))}
          </div>
        </div>
        ...
        <div ref={r4.ref} className={`mt-8 card-dark flex items-start gap-4 ${r4.cls}`} style={{ borderLeft: '3px solid var(--accent)' }}>
          <Icon name="zap" size={20} style={{ color: 'var(--accent)' }} className="shrink-0 mt-1" />

    ...
    <div className={`rounded-xl overflow-hidden flex flex-col h-full lift ${warn ? 'bg-red-950/30 border border-red-500/15' : 'bg-white/50 border border-warm-200'}`}>
      <div className="p-5 flex flex-col flex-1">
        <div className="flex items-start justify-between mb-2">
          <span className={`font-mono text-[8px] tracking-[0.2em] uppercase ${warn ? 'text-red-400/50' : 'text-dark/30'}`}>{b.geo}</span>
          {b.badge && <span className="font-mono text-[9px] text-green-600 bg-green-50 px-1.5 py-0.5 rounded-full">{b.badge}</span>}
        </div>
        ...
        <div className={`mt-3 py-2.5 px-3 rounded-lg ${warn ? 'bg-red-500/8' : 'bg-warm-50'}`}>
          ...
          <p className={`font-mono text-[9px] mt-0.5 ${warn ? 'text-red-300/40' : 'text-dark/35'}`}>{b.headlineSub}</p>
        </div>
        <div className="mt-3 flex flex-wrap gap-1">
          {b.details.map((d, i) => <span key={i} className={`px-1.5 py-0.5 rounded text-[9px] ${warn ? 'bg-red-500/5 border border-red-500/10 text-red-300/60' : 'bg-dark/4 border border-dark/6 text-dark/50'}`}>{d}</span>)}
        </div>
    ...
    <section className="section-light">
      <div className="section-pad">
        <div ref={r1.ref} className={r1.cls}>
          <span className="font-mono text-[10px] tracking-[0.35em] uppercase text-terracotta/50">03</span>
        ...
        <div className="mt-8 border border-warm-200 rounded-xl p-5 bg-white/30 flex items-start gap-3">
          <div className="w-9 h-9 rounded-lg bg-dark flex items-center justify-center shrink-0"><span className="font-mono text-[10px] text-terracotta-light font-bold">IN</span></div>
          <div>
